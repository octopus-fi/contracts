...
public entry fun ai_claim_and_rebalance<T>(
    ...
    let rewards = liquid_staking::claim_rewards_to_vault(pool, position, clock, ctx);
...
public fun check_rebalance_needed<T>(vault: &Vault<T>, oracle: &Oracle): (bool, u64, u64) {
    let debt = vault_manager::get_debt(vault);
    let collateral = vault_manager::get_collateral(vault);
...
public fun estimate_rebalance_effect<T>(vault: &Vault<T>, oracle: &Oracle): (u64, u64) {
    let debt = vault_manager::get_debt(vault);
    let collateral = vault_manager::get_collateral(vault);
    let reserve = vault_manager::get_reward_reserve(vault);
    let price = oracle_adapter::get_price<T>(oracle);
