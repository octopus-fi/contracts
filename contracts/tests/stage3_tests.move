...
use octopus_finance::strategy_registry::{Self, Registry};
...
fun test_walrus_strategy_registration() {
    ...
    test_scenario::next_tx(&mut scenario, ADDR_USER);
    {
        ...
        let blob_id = b"walrus_blob_123456789";
        let name = b"Max Yield Strategy";
...
fun test_liquidation_simulation() {
    ...
    // Take StakePosition (created by staking)
    test_scenario::next_tx(&mut scenario, ADDR_USER);
    {
    ...
    test_scenario::next_tx(&mut scenario, ADDR_USER);
    {
        let mut registry = test_scenario::take_shared<vault_manager::VaultRegistry<OCTSUI>>(&scenario);
        vault_manager::create_vault<OCTSUI>(&mut registry, test_scenario::ctx(&mut scenario));
        test_scenario::return_shared(registry);
    };

    // 9. User Deposits octSUI Collateral
    test_scenario::next_tx(&mut scenario, ADDR_USER);
    {
    ...
    test_scenario::next_tx(&mut scenario, ADDR_USER);
    {
        let vault = test_scenario::take_from_sender<Vault<OCTSUI>>(&scenario);
        let oracle = test_scenario::take_shared<Oracle>(&scenario);
    ...
    test_scenario::next_tx(&mut scenario, ADDR_USER);
    {
        ...
        let repay_coin = coin::split(&mut octusd_coin, 100_000_000_000, test_scenario::ctx(&mut scenario));
        
        transfer::public_transfer(repay_coin, ADDR_LIQUIDATOR);
        test_scenario::return_to_sender(&scenario, octusd_coin);
    };

    // 15. Liquidator executes liquidation!
    test_scenario::next_tx(&mut scenario, ADDR_LIQUIDATOR);
    {
        let mut vault = test_scenario::take_from_address<Vault<OCTSUI>>(&scenario, ADDR_USER);
        let mut bank = test_scenario::take_shared<Bank>(&scenario);
        let oracle = test_scenario::take_shared<Oracle>(&scenario);
        ...
        test_scenario::return_to_address(ADDR_USER, vault);
        test_scenario::return_shared(oracle);
        test_scenario::return_shared(bank);
    };
...
fun test_ai_rebalance_with_reserve() {
    ...
    {
        let mut mocksui_cap = test_scenario::take_from_sender<TreasuryCap<MOCKSUI>>(&scenario);
        mocksui::mint(&mut mocksui_cap, 200_000_000_000, ADDR_USER, test_scenario::ctx(&mut scenario)); // 200 mSUI
        test_scenario::return_to_sender(&scenario, mocksui_cap);
    };
    ...
    test_scenario::next_tx(&mut scenario, ADDR_USER);
    {
        ...
        let reserve_portion = coin::split(&mut all_octsui, 100_000_000_000, test_scenario::ctx(&mut scenario));
...
fun test_ai_claim_rewards_and_rebalance() {
    ...
    {
        let mut mocksui_cap = test_scenario::take_from_sender<TreasuryCap<MOCKSUI>>(&scenario);
        mocksui::mint(&mut mocksui_cap, 1000_000_000_000, ADDR_USER, test_scenario::ctx(&mut scenario)); // 1000 mSUI
        test_scenario::return_to_sender(&scenario, mocksui_cap);
    };
    ...
    {
        let vault = test_scenario::take_from_sender<Vault<OCTSUI>>(&scenario);
        let mut position = test_scenario::take_from_sender<StakePosition>(&scenario);
        
        // Enable auto-rebalance and link to vault
        liquid_staking::enable_auto_rebalance(&mut position, object::id(&vault), test_scenario::ctx(&mut scenario));
    ...
    test_scenario::next_tx(&mut scenario, ADDR_AI_AGENT);
    {
        let ai_cap = test_scenario::take_from_sender<AICapability>(&scenario);
        let mut pool = test_scenario::take_shared<StakingPool<MOCKSUI>>(&scenario);
        let mut position = test_scenario::take_from_address<StakePosition>(&scenario, ADDR_USER);
        let mut vault = test_scenario::take_from_address<Vault<OCTSUI>>(&scenario, ADDR_USER);
        let oracle = test_scenario::take_shared<Oracle>(&scenario);
        ...
        test_scenario::return_shared(pool);
        test_scenario::return_to_address(ADDR_USER, vault);
        test_scenario::return_to_address(ADDR_USER, position);
        test_scenario::return_to_sender(&scenario, ai_cap);
    };

    // 16. Verify AI used staking rewards for rebalancing
    // Note: In test scenario, epoch doesn't advance so rewards may be 0
    // But the flow works - in production, epochs advance and rewards accrue
    test_scenario::next_tx(&mut scenario, ADDR_USER);
    {
        let vault = test_scenario::take_from_sender<Vault<OCTSUI>>(&scenario);
        let position = test_scenario::take_from_sender<StakePosition>(&scenario);
        ...
        // In real usage with epoch advancement, rewards would be claimed and used
